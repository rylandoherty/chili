...
function ProductCtrl($routeParams, $scope, $window, ngclipboard) {
	...
	$scope.quickOrderFunction = function (x,storeSelection){
		
		if( x[storeSelection].quickorder > 0){
			x[storeSelection].trueorder=  x[storeSelection].quickorder;
			x[storeSelection].addToCart=1;
			x[storeSelection].ordercost = x[storeSelection].quickorder * x.cost;
			console.log(x[storeSelection]);
		}
		else{
			x[storeSelection].trueorder=x[storeSelection].order;
			x[storeSelection].addToCart=0;
			x[storeSelection].ordercost= x[storeSelection].trueorder * x.cost;
		}
	...
	$scope.productList[products].color = $scope.getColor($scope.productList[products]['name']);
 				...
 				for(var stores in $scope.storeArray){
 					...
 					$scope.productList[products][$scope.storeArray[stores].storeid]['quickorder'] = 0;
