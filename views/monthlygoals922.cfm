  ...
  margin: 0 0 0 0.5em;
			...
			<body>
				...
				<select ng-if="thisGuy.accesslevel < 2" ng-model ="selectedEmployee.name" ng-change="updateGuy()" >
					<option ng-repeat = "users in employeeListDD" ng-value ="users.userid"> {{users.userid}}</option>
				 </select>
				<div >Commission Earned: {{ rightcontainer['MTD'].employeesbyname[selectedEmployee.name].forms['GrossProfit'].value * (selectedEmployee['data'].basepercent + bonusRate)/100|number : 2 }}  <br>
				 ...
				 <div>Trending Commission: {{ (rightcontainer['MTD'].employeesbyname[selectedEmployee.name].forms['GrossProfit'].value*trend) * (selectedEmployee['data'].basepercent + bonusRateTrend)/100|number : 2 }}  <br> 
...
<td class="current bar" ng-style=" {'height': ((rightcontainer['MTD'].employeesbyname[selectedEmployee.name].forms[goallist].value/empbystorewithstats[selectedEmployee.name][goallist]['forcast'])*200)+'px'}">
	<p style=" color: white; font-weight: bolder ">{{rightcontainer['MTD'].employeesbyname[selectedEmployee.name].forms[goallist].value| number : goallist.decimal}}</p></td>


<td class="trend bar" ng-if="groupsbyname[goallist].trending" ng-style=" {'height': (((rightcontainer['MTD'].employeesbyname[selectedEmployee.name].forms[goallist].value*trend)/empbystorewithstats[selectedEmployee.name][goallist]['forcast']).toFixed(2))*200+'px'} ">
	
	<p style=" color: white; font-weight: bolder ">{{rightcontainer['MTD'].employeesbyname[selectedEmployee.name].forms[goallist].value*trend |number : 2}}</p> </td>
...
<div> 
	{{goallist}}<br>
Earn : {{graphData[goallist].ratecurrent|number:2}}/{{graphData[goallist].rate}} --- <span ng-if="groupsbyname[goallist].trending"> Trend   : {{graphData[goallist].ratetrend|number:2}}/{{graphData[goallist].rate}}  </span><br> 

Progress	 {{graphData[goallist].progress*100 |number:3 }}%  
	 <br>
	 <span ng-if="groupsbyname[goallist].trending"> Trend Progress	 {{graphData[goallist].progresstrend*100 |number:4 }}%</span><br>
	 </div>
</div>
