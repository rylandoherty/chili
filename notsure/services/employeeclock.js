    	...
    	totalObject.weekArray[dateToForm(currentStartOfWeek)].thisWeekStart = dateToForm(currentStartOfWeek);
    		...
    		if(thisDate<=currentEndOfWeek&&thisDate>=currentStartOfWeek){
	    		...
	    		hourTable[list2[entries].username].hoursworked =
	    		 ...
	    		 else if(hourTable[list2[entries].username].hoursworked<=40){
    		...
    		if(thisDate<=currentEndOfWeek&&thisDate>=targetDate && weekcount == 1){
    			...
    			if(isholiday){
	    				hourTable[list2[entries].username].holidayfallintodraw = list2[entries].hours;
	    		 }
	    		 else{
    		...
    		if(weekcount == 1){ 
    			for(var each in hourTable ){
    				hourTable[each].drawforweek = hourTable[each].holidayfallintodraw * 18;
    				if(hourTable[each].hoursworked<=40){
    					hourTable[each].drawforweek = hourTable[each].drawforweek + ((hourTable[each].hoursfordraw - hourTable[each].holidayfallintodraw) * 12);
    				}
    				else{
    					hourTable[each].drawforweek = hourTable[each].drawforweek +((hourTable[each].hoursfordraw - hourTable[each].holidayfallintodraw - hourTable[each].overtimehours)*12)+(hourTable[each].overtimehours*18);
    				}
