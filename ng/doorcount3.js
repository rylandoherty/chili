...
function DoorCountCtrl($routeParams, $scope, $window) {
	...
	var monthNames = ["January", "February", "March", "April", "May", "June",
 	...
 	$scope.convertCSVintoString = function(){
 		
 		$scope.groups = groupProxy.loadFormula();
 		for (var formulas in $scope.groups){
 			var incomingstring = $scope.groups[formulas].formula;
 			var stringFormula = "";
		...
		$scope.groups[formulas].formula = stringFormula;
		$scope.formContainer.push($scope.groups[formulas]);	
	}
	...
	$scope.postSalesOp = function(){
		...
		for(var emps in container.employees){
			for(var formula in $scope.formContainer){
				$scope.container.employees[emps]['stats']['formulagroup'][form[formula]]
			}
			
			
		}
		for(var store in container.stores){
